...
local __topblack = 0x010409
local __textcolor = 0xC9D9D9
local __white = 0xFFFFFF
local __listbg = 0x141B24
local __bg = 0x0D1117
local __button = 0x21262D
local __hovered = 0x30363D
local __disabletext = 0x68707A
local __selected = 0x2C333C
 
local colorScreenBackground = __bg
local colorScreenForeground = __white
local colorTopLineBackground = __topblack
local colorTopLineForeground = __white
local colorBottomLineBackground = __topblack
local colorBottomLineForeground = __white
local colorFrameBackground = __bg
local colorFrameForeground = __white
local colorButtonBackground = __button
local colorButtonForeground = __white
local colorButtonClickedBackground = __hovered
local colorButtonClickedForeground = __white
local colorButtonDisabledBackground = __hovered
local colorButtonDisabledForeground = __disabletext
local colorTextBackground = __button
local colorTextForeground = __white
local colorInputBackground = __topblack
local colorInputForeground = __white
local colorProgressBackground = 0x000000
local colorProgressForeground = 0x00FF00
local colorProgressNumberForeground = 0xFFFF00
local colorListBackground = __button
local colorListForeground = __textcolor
local colorListActiveBackground = __selected
local colorListActiveForeground = __white
local colorListDisabledBackground = __button
local colorListDisabledForeground = __disabletext
...
function gui.setTop(text)
  gpu.setBackground(colorTopLineBackground)
  gpu.setForeground(colorTopLineForeground)
  gpu.set( (screenWidth / 2) - (unicode.wlen(text) / 2), 1, text)
end
 
function gui.setBottom(text)
  gpu.setBackground(colorBottomLineBackground)
  gpu.setForeground(colorBottomLineForeground)
  gpu.set( (screenWidth / 2) - (unicode.wlen(text) / 2), screenHeight, text)
end
...
--    os.sleep(1)
  ...
  if guiID.frame == true then
    ...
    if guiID.text then
      gpu.set(guiID.x + math.floor((guiID.width/2)) - math.floor((unicode.wlen(guiID.text)/2)), guiID.y, guiID.text)
    end
  end
end
...
local function _displayAFrame(guiID, frameID)
  if guiID[frameID].visible == true then
    ...
    if guiID[frameID].text then
      gpu.set(guiID[frameID].x + math.floor((guiID[frameID].width/2)) - math.floor((unicode.wlen(guiID[frameID].text)/2)+1), guiID[frameID].y, "╡" .. guiID[frameID].text .. "┝")
    end
  end
end
...
local function _displayLabel(guiID, labelID)
  if guiID[labelID].visible == true then
    ...
    if guiID[labelID].x == "center" then
      x = guiID.x + math.floor((guiID.width / 2)) - math.floor((unicode.wlen(guiID[labelID].text)) / 2)
    else
...
local function _displayText(guiID, textID)
  if guiID[textID].visible == true then
    ...
    if guiID[textID].hide == true then
      tmpStr = ""
      for i = 1, unicode.wlen(guiID[textID].text) do
    tmpStr = tmpStr .."*"
      end
    end
...
local function _displayList(guiID, listID)
  if guiID[listID].visible == true then
    ...
    if guiID[listID].text then
      gpu.set( guiID[listID].x + (guiID[listID].width/2) - (unicode.wlen(guiID[listID].text)/2), guiID[listID].y, "╡" .. guiID[listID].text .. "┝")
    end
...
function gui.newLabel(guiID, x, y, text, bg, fg, l)
  ...
  tmpTable["y"] = y + guiID.y
  tmpTable["text"] = text
  tmpTable["lenght"] = unicode.wlen(text)
  tmpTable["bg"] = bg or guiID.bg
  tmpTable["fg"] = fg or guiID.fg
  tmpTable["visible"] = true
  tmpTable["x"] = x
  tmpTable["l"] = l or unicode.wlen(text)
  table.insert(guiID, tmpTable)
  return #guiID
end
...
function gui.newButton(guiID, x, y, text, func)
  ...
  tmpTable["y"] = y + guiID.y
  tmpTable["text"] = " " .. text .. " "
  tmpTable["lenght"] = unicode.wlen(tmpTable.text)
  tmpTable["visible"] = true
  tmpTable["enabled"] = true
...
function characterValidation(character)
  if character then
    if character > 31 and character < 128 then return true end
    if character > 1039 and character < 1105 then return true end
  end
...
local function runInput(guiID, textID)
  ...
  while loopRunning == true do
    ...
    if guiID[textID].hide == true then
      tmpStr = ""
      for i = 1, unicode.wlen(inputText) do
    tmpStr = tmpStr .."*"
      end
    end
    if unicode.wlen(tmpStr) + 1 > guiID[textID].fieldLenght then
      tmpStr = string.sub(tmpStr, unicode.wlen(tmpStr) - guiID[textID].fieldLenght + 2, unicode.wlen(tmpStr))
    end
    ...
    if e == "key_down" then
      if character == 8 then    -- backspace
        local temp = unicode.wlen(inputText) > 1 and string.byte(string.sub(inputText, -1)) > 127 and 2 or 1
    inputText = string.sub(inputText, 1, unicode.wlen(inputText) - temp)
    ...
    loopRunning = false
      elseif characterValidation(character) and unicode.wlen(inputText) < guiID[textID].lenght then
        if character > 128 then
          inputText = inputText .. utf8.char(character)
        else
...
-- function gui.yesNoCallbackClose()
--   yesNoRunning = false
--   yesNoValue = false
...
yesNoYesButton = gui.newButton(yesNoGui, 3, 8, "Да", yesNoCallbackYes)
yesNoNoButton = gui.newButton(yesNoGui, 33, 8, "Нет", yesNoCallbackNo)
 
 
function gui.getYesNo(msg1, msg2, msg3, msgyes, msgno)
  yesNoRunning = true
  ...
  if msgyes then gui.setText(yesNoGui, yesNoYesButton, msgyes) end
  if msgno then gui.setText(yesNoGui, yesNoNoButton, msgno) end
...
function gui.runGui(guiID)
  ...
  if ev == nil or ev[1] == nil then
    return false
  end

  if ev[1] == "player_off" then
    gui.pimPlayer = nil
    yesNoCallbackNo()
    msgCallback()
